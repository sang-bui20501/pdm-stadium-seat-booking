...
function ShowBookings() {
  ...
  const getBookingList = async () => {
    axios
      .get(`http://localhost:8080/customer/getBookings/${cookies.userId}`)
      .then((response) => {
        if (response.data) {
  ...
  const handleDelete = (e, id) => {
    e.preventDefault();
    axios
      .delete(`http://localhost:8080/booking/remove/${id}`)
      .then((response) => {
        if (response.data) {
        ...
        arr.push(
          <div className="show-booking-item">
            <div className="show-booking-item-general-info">
              <p className="show-booking-item-title">
                Booking #{item.booking_id}
              </p>
              <button
                className="show-booking-pay-btn"
                onClick={(e) => handlePayment(e, item.booking_id)}
              >
                Pay for booking
              </button>
            </div>
            <div className="show-booking-item-info">
              <p className="show-booking-info-item">
                Seat info: Seat {item.id} {item.type}
              </p>
              <p className="show-booking-info-item">
                Booking date: {getDate(item.start_time)}
              </p>
              <p className="show-booking-info-item">
                Booking time: {getTime(item.start_time)} -{" "}
                {getTime(item.end_time)}
              </p>
              <p className="show-booking-info-item">
                Price: {item.rate * item.duration}
              </p>
          ...
          <div className="show-booking-item">
            <div className="show-booking-item-general-info">
              ...
              <button
                ...
                onClick={(e) => handleDelete(e, item.booking_id)}
              >
                Delete
              </button>
            </div>
  ...
  return (
    <div className="show-booking-wrapper">
      ...
      <div className="show-booking-list">{listBookings()}</div>
    </div>
  );
}
