...
function BookingPage() {
  ...
  const maxDate = new Date(Date.now() + 60 * 60 * 24 * 30 * 1000)
    .toISOString()
    .split("T")[0];
  ...
  const handleClick = async (e) => {
    ...
    axios
      .post("http://localhost:8080/booking/getavailableseats", form)
      .then((response) => {
        if (response.data) {
  ...
  const handleSubmit = async () => {
    ...
    axios
      .post(`http://localhost:8080/booking/save/${customerId}`, form)
      .then((response) => {
        if (response.data) {
    ...
    for (let i in availableSeats) {
      arr.push(
        <td className="booking-page-td">
          <button
            className="booking-page-seat-choice"
            value={availableSeats[i]}
            onClick={() => setSelectedSeat(availableSeats[i])}
          >
            Seat {availableSeats[i].id}
          </button>
        </td>
      );
    }
        ...
        <div className="booking-page-form">
          ...
          <table className="booking-page-table">
            <tbody>
                ...
                <tr>
                  <td className="booking-page-td">
                    <input
                      ...
                      onChange={() => {
                        setBookingType("Seat");
                        setShowSeatType(true);
                        setShowFormInput(true);
                      }}
                    />
                    <label for="seat" className="booking-page-booking-type">
                      Seat
                    </label>
                    <input
                      type="radio"
                      ...
                      value="stadium"
                      onChange={() => {
                        setBookingType("Stadium");
                        setShowFormInput(true);
                        setShowSeatType(false);
                        setSeatId(null);
                        setPriceId(3);
                      }}
                    />
                    <label for="stadium" className="booking-page-booking-type">
                      Stadium
                    </label>
                  </td>
                </tr>
              ...
              <div
                className="booking-page-form-section"
                style={{ display: showSeatType ? "block" : "none" }}
              >
                <tr>
              ...
              <div
                className="booking-page-form-input-div"
                style={{ display: showFormInput ? "block" : "none" }}
              >
                <div className="booking-page-form-section">
                  <tr>
                ...
                >
                  <tr>
                    <td className="booking-page-td">
                      <button
                        className="booking-page-choose-seat"
                ...
                <div className="booking-page-form-section">
                  ...
                  <tr>
                    <td className="booking-page-td">
                      <input
                        type="text"
                        className="booking-seat-price"
                        name="price"
                        value={
                          showSeatType
                            ? seatPrice * bookingDuration
                            : stadiumPrice * bookingDuration
                        }
                        readOnly={true}
                      />
                    </td>
          ...
          <input
            className="booking-page-submit-btn"
            type="submit"
            value="Submit"
